...
var snelheid=0, vervoer='', weg='', totaleAfstand=0, counter=0;
...
// de functie die kijkt welk vervoermiddel je gebruikt en je vervolgens naar de juiste functie doorstuurt
if (vervoer == "voet") {
    voetRegels();
     }	else {
        if (vervoer == "fiets") {
...
// de functie voor als je te voet gaat. Hij stuurt je erna door naar de functie meldstatus
function voetRegels() {
    if (weg != "stoep") {
...
// de functie voor als je te fiets gaat. Hij stuurt je erna door naar de functie meldstatus
...
// de functie voor als je te auto gaat. Hij stuurt je erna door naar de functie meldstatus
function autoRegels() {
    if (weg == "stoep") {
...
function meldStatus(tekst) {
    //totale afstand berekenen, counter bijhouden en gemiddelde per uur berekenen
    totaleAfstand = parseInt(totaleAfstand) + parseInt(snelheid);
    counter++;
    var gemiddeld = totaleAfstand / counter;

    $("#status").html(tekst);
    document.getElementById('status').innerHTML += 'U gaat over de ' + weg + ' te ' + vervoer + ' met een snelheid van: ' + snelheid + ' km/u <br /> U gaat in ' + counter + " uur " + totaleAfstand + " kilometer. Dat is " + gemiddeld + " kilometer per uur. <br/>" ;
}